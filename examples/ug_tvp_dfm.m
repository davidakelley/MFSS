% Mixed-frequency dynamic factor model with TVP variance
% This is vaguely inspired by the BBKI model. There are two latent factors
% that have fixed loadings on the low-frequency series, which also has no
% observation errors. 
%
...
%% Estimating a MF-DFM with a TVP for volatility
...
Z = [ones(5,1), [1; nan(4,1)]];
d = [nan; zeros(4,1)];
H = diag([0; nan(4,1)]);
T = diag(nan(2,1));
Q = struct('Qt', nan(2,2,2), 'tauQ', [ones(500,1); 2 .* ones(119+1,1)]);
...
plot(dates, zscore(alphaHat(:,2)), 'r', 'LineWidth', 0.5)
datetick('x');
recessionplot
xlim(dates([1 end]))
ylim([-6 6])
title('Monthly factors and data (z-scores)');

% Quarterly data (GDP) with the aggregated (quarterly) factors
subplot(2, 1, 2);
hold on
plot(dates(3:3:end), sum(alphaHat(3:3:end,5:6),2) + ssML.d(1), 'k', 'LineWidth', 2)
plot(dates, y(:,1), '.')
datetick('x');
recessionplot
xlim(dates([1 end]))
