...
\section{Gradient Derivation} \label{sec:gradient_derivation}
	...
	The chain rule for the operator $\Gt(\cdot)$ is simply $\Gt(A) = \Gt(\psi) G_\psi(A)$ where $A$ is a function of $\psi$, which is itself a function of $\theta$. This can be shown by noting that 
	\begin{equation*}
